...
================================================================================
ROBUSTNESS CHECK: Period-by-Period Event Study — Z-scored AIIE
================================================================================
...
Identical to 06_2021start.py except aiie_zscore replaces aiie_score as the
treatment variable. Coefficients are rescaled to represent the effect of a
1 standard deviation increase in industry AI exposure rather than a 1-unit
increase in raw AIIE.
================================================================================

MODEL
-----
  log y_it = α + Σ_τ β_τ·(aiiez_i × 1[t=τ]) + γ_i + δ_t + ε_it
  ...
  aiiez_i = Felten AIIE score, z-scored across supersectors
            (mean=0, SD=1; β_τ interpreted as effect per 1 SD increase in AIIE)
...
OUTPUTS
-------
  results/robust_21z_log_emp.csv      coefficient table, employment
  results/robust_21z_log_wage.csv     coefficient table, wages
  results/robust_21z_log_wbill.csv    coefficient table, wage bill
  results/robust_21z_log_emp.png      event study plot, employment
  results/robust_21z_log_wage.png     event study plot, wages
  results/robust_21z_log_wbill.png    event study plot, wage bill
================================================================================
"""
...
def build_interactions(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    for tau in taus:
        ...
        df[col] = df["aiie_zscore"] * (df["event_time"] == tau).astype(float)
...
def estimate_event_study(
    ...
    sample = sample.dropna(subset=[outcome, "aiie_zscore"])
...
def plot_event_study(
    ...
    ax.set_title(
        ...
        f"Effect of Felten AIIEz × Quarter on {outcome_label} "
...
def main() -> None:
    ...
    for outcome, label in OUTCOMES.items():
        ...
        csv_path = RESULTS_DIR / f"robust_21z_{outcome}.csv"
        ...
        # Plot
        png_path = RESULTS_DIR / f"robust_21z_{outcome}.png"
